...
{
	std::ofstream myfile;
	// myfile.open("slouchstraight2.txt", std::ios::app);
	myfile.open("spincmd.txt");
	myfile << "0";
	myfile.close();
...
void CSkeletonBasics::ProcessSkeleton()
{
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
		...
		//	skeletonFrame.SkeletonData[0].SkeletonPositions
		if (NUI_SKELETON_TRACKED == trackingState)
        {
			...
			//printf("assdds\n");
			if (curdir == 0) {
				if (skeletonFrame.SkeletonData[i].Position.x > 0.3) {
					curdir = 1;
					std::ofstream myfile;
					// myfile.open("slouchstraight2.txt", std::ios::app);
					myfile.open("spincmd.txt");
				...
				else if (skeletonFrame.SkeletonData[i].Position.x < -0.3) {
					curdir = -1;
					std::ofstream myfile;
					// myfile.open("slouchstraight2.txt", std::ios::app);
					myfile.open("spincmd.txt");
				...
				if (skeletonFrame.SkeletonData[i].Position.x < 0.15 && skeletonFrame.SkeletonData[i].Position.x > -0.15) {
					curdir = 0;
					std::ofstream myfile;
					// myfile.open("slouchstraight2.txt", std::ios::app);
					myfile.open("spincmd.txt");
...
{   
	...
	if ((abs(m_Points[NUI_SKELETON_POSITION_HEAD].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x) > 30) || (abs(m_Points[NUI_SKELETON_POSITION_SPINE].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x) > 30)) {
		// write bad posture
		if (post != 0) {
			std::ofstream myfile;
			myfile.open("postcmd.txt");
			myfile << "0";
			myfile.close();
		}
	}
	...
	/*
	myfile
		<< m_Points[NUI_SKELETON_POSITION_HEAD].x << ","
		<< m_Points[NUI_SKELETON_POSITION_HEAD].y << ","
		<< m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x << ","
		<< m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].y << ","
		<< m_Points[NUI_SKELETON_POSITION_SPINE].x << ","
		<< m_Points[NUI_SKELETON_POSITION_SPINE].y << ","
		<< m_Points[NUI_SKELETON_POSITION_HIP_CENTER].x << ","
		<< m_Points[NUI_SKELETON_POSITION_HIP_CENTER].y << "\n";
	*/
	//myfile.close();
